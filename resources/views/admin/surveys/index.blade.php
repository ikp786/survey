                    ...
                    <table class="table all-type-ques" id="dataTable_2">
                        <thead>
                            <tr>
                                ...
                                <th scope="col">Survey Complete</th>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>{{$val->number_of_attempt}}</td>
                                <td>{{$val->is_complete == 1 ? 'Complete' : 'Pending'}}</td>
                                <td>{{$val->is_email_sent == 1 ? 'Sent' : 'Pending'}}</td>
                                <td>{{$val->created_at}}</td>
                                <td class="delete-edit-btn">
                                    @if($val->is_complete == 1)
                                    <a class="btn-xs sharp me-1" href="{{ route('admin.survey.result',$val->id) }}"><i class="fas fa-eye text-secondary" aria-hidden="true"></i></a>
                                    @else
                                    Pending
                                    @endif
                                </td>
